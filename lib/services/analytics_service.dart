...
class AnalyticsService {
  ...
  Future<List<Map<String, dynamic>>> getProfitTrendsForPeriod(String period) async {
    final dateRange = _getDateRangeForPeriod(period);
    ...
    for (final transaction in transactions) {
      ...
      dailyProfit[dateKey] = (dailyProfit[dateKey] ?? 0) + transaction.totalProfit;
    }
  ...
  Future<List<Map<String, dynamic>>> getTopProductsForPeriod(String period) async {
    final dateRange = _getDateRangeForPeriod(period);
    final transactions = await _transactionRepository.getTransactionsByDateRange(
      dateRange['start']!,
      dateRange['end']!,
    ...
    for (final transaction in transactions) {
      final items = await _transactionRepository.getTransactionItems(transaction.id!);
  ...
  Future<Map<String, double>> getExpensesByCategoryForPeriod(String period) async {
    final dateRange = _getDateRangeForPeriod(period);
  ...
  Future<int> getLowStockProductsCount({int threshold = 10}) async {
    final products = await _productRepository.getLowStockProducts(threshold: threshold);
